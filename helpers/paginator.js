exports.paginator = async (
  ...
  if (p && l) {
    ...
    if (q) {
      // query with certain fields
      let query = {}
      for (let pair of q.split(',')) {
        const splitted = pair.split('=')
        if (splitted[1].trim() === 'true' || splitted[1].trim() === 'false')
          query[splitted[0].trim()] = splitted[1].trim()
        else if (splitted[0].trim() === '_id')
          query[splitted[0].trim()] = splitted[1].trim()
        else
          query[splitted[0].trim()] = {
            $regex: new RegExp(splitted[1].trim(), 'i'),
          }
      }
...
exports.promisePaginator = async (queryObject, promise, model) => {
  let items
  const { p = 1, l = 50, q, f = '' } = queryObject // f is for projection
  let limit = Number(l)
  const page = Number(p)
